...
```bash
python cn_run.py -p "<자립형 지시문>"
```

**반드시 프로젝트 루트(= `.env`가 있는 디렉터리)에서 실행한다.** cn이 워크스페이스 루트 `.env`에서 `OPENROUTER_API_KEY`를 찾기 때문 — 다른 cwd에서 실행하면 시크릿 해석이 안 된다. (config 파일 자체는 스킬 폴더에 있어도 무관 — cn이 `--config`로 명시된 경로를 그대로 읽고, 시크릿만 cwd 기준으로 찾는다.)
...
| 사용자 발화 | 동작 |
|---|---|
| (이미 위임이 켜진 상태에서) 그냥 위임 지시만 | `cn_run.py -p "..."` — 현재 sticky/autoscale 그대로 |
...
5. 비용 가드·diff 검증 원칙은 위 "모델 선택"/"기본 사용"과 동일하게 적용.
