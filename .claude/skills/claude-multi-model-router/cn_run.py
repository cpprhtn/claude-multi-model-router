...
사용 (프로젝트 루트에서):
  ...
  python cn_run.py --tier mid -p "..."                # Claude가 이미 정한 ladder 티어로 직접 위임
  ...
  python cn_run.py --allow-dirty -p "..."             # working tree가 dirty해도 강행(기본은 차단, B-6)
  python cn_run.py --readonly -p "..."                # 코드 리뷰/교차검증처럼 파일 수정이 필요 없는 요청 전용.
                                                       #   --auto 대신 --readonly(읽기 전용 도구만 허용)로 호출
                                                       #   — --auto(전체 도구 허용)는 에이전트가 프롬프트에
                                                       #   붙여넣은 코드보다 "직접 파일을 열어 확인"하려는 쪽으로
                                                       #   편향되어, 도구 호출이 애매해지면 모델 무관(opus 포함
                                                       #   재현됨)하게 "코드가 제공되지 않았다"고 오응답하는 사례가
                                                       #   관찰됨. dirty 검사도 건너뜀(파일을 안 건드리므로).
...
from route import (  # noqa: E402 — route.py의 __main__ 블록은 실행 안 됨
    classify, FREE_KW, LADDER_TIERS, load_roster, read_sticky, write_sticky, clear_sticky,
...
def active_model(roster, override=None, autoscale_prompt=None, forced_ladder_tier=None):
    """우선순위 — override(--model 1회성 지정) > sticky >
    forced_ladder_tier(Claude가 SKILL.md 표에서 이미 정한 --tier, classify() 건너뜀) >
    autoscale(classify() 폴백) > free_slug."""
    if override:
...
def is_git_dirty():
    """working tree가 clean한지 확인(B-6 안전망). git repo가 아니면 검사 자체를 건너뛴다
    (None 반환) — cn 위임이 git 없는 프로젝트에서도 동작해야 하므로."""
    try:
...
def cmd_explain(prompt, roster, override, forced_ladder_tier=None):
    slug, tier, note = active_model(roster, override, autoscale_prompt=prompt,
                                     forced_ladder_tier=forced_ladder_tier)
    sticky = read_sticky()
    print(f"[설명] override={override!r} sticky={sticky!r} tier={forced_ladder_tier!r} "
          f"오토스케일={'on' if read_autoscale() else 'off'}")
...
def cmd_run(prompt, roster, override, base_config_arg, extra_format,
            forced_ladder_tier=None, allow_dirty=False, readonly=False):
    # --auto(모든 도구 허용)는 에이전트가 프롬프트에 붙여넣은 코드보다 "직접 파일을 열어
    # 확인해야 한다"는 쪽으로 편향되어, 리뷰/분석처럼 파일 수정이 필요 없는 요청에서
    # 도구 호출이 애매해지면 "코드가 없다"고 응답하는 사례가 관찰됨(모델 무관, opus 포함
    # 재현됨). readonly=True면 --readonly(읽기 전용 도구만 허용)로 전환해 이 문제를 피한다.
    # 파일을 건드리지 않으므로 dirty working tree 검사도 적용하지 않는다.
    if not readonly:
        dirty = is_git_dirty()
        if dirty and not allow_dirty:
            print("[중단] working tree가 clean하지 않습니다(커밋되지 않은 변경 있음). cn 위임(--auto)은 "
                  "기존 변경과 뒤섞일 수 있어 기본적으로 막혀 있습니다 — 변경분을 커밋/스태시하거나, "
                  "위험을 인지하고 진행하려면 --allow-dirty를 추가하세요.", file=sys.stderr)
            sys.exit(3)
    ...
    cmd = [cn_path, "--config", config_path, "--readonly" if readonly else "--auto", "-p", prompt]
    if extra_format:
        cmd += ["--format", extra_format]

    if os.name == "nt":
        # Windows npm 글로벌 설치는 cn.cmd(배치 스텁)라 CreateProcess가 직접 실행 못 해
        # cmd.exe(shell=True)를 거쳐야 한다고 알려져 있었으나, 그 경로는 실측 결과 버그가
        # 있다: 프롬프트에 큰따옴표(")가 하나라도 있으면(리뷰 요청 텍스트에는 흔함)
        # list2cmdline이 CreateProcess 인자 파싱용으로 이스케이프해도 cmd.exe 자체의
        # 명령줄 파싱(따옴표 추적)이 그 지점에서 깨져, 이후 내용(예: C++ 코드의 `&` 참조
        # 연산자)이 셸 메타문자로 해석되면서 실제 모델에는 프롬프트가 잘리거나 아예
        # 전달되지 않았다(모델 무관 — opus/sol/terra 전부 재현, cn을 직접 호출하면
        # 항상 성공하는 것으로 확인). 그래서 cn.cmd가 감싸는 실제 진입점
        # (node_modules/@continuedev/cli/dist/cn.js)을 node.exe로 직접 호출해
        # cmd.exe 자체를 경유하지 않는다 — shell=False + argv 리스트라 Python이
        # CreateProcess용으로만 인자를 인코딩하고, cmd.exe의 셸 메타문자 재해석이 없다.
        node_cmd = _resolve_node_direct_invocation(cn_path)
        if node_cmd:
            result = subprocess.run(node_cmd + cmd[1:])
        else:
            # cn.js를 못 찾으면(설치 구조가 다른 경우) 기존 cmd.exe 경로로 폴백한다 —
            # 프롬프트에 큰따옴표가 없을 때는 문제없이 동작한다.
...
def _resolve_node_direct_invocation(cn_cmd_path):
    """cn.cmd(npm 배치 스텁)가 감싸는 실제 node 진입점을 찾아 [node.exe, cn.js]를 반환한다.
    못 찾으면 None(호출부가 기존 cmd.exe 폴백을 쓰게 함)."""
    base_dir = os.path.dirname(cn_cmd_path)
    cn_js = os.path.join(base_dir, "node_modules", "@continuedev", "cli", "dist", "cn.js")
    if not os.path.exists(cn_js):
        return None
    node_exe = shutil.which("node")
    if not node_exe:
        return None
    return [node_exe, cn_js]
...
def main():
    ...
    while i < len(args):
        ...
        if a in ("-p", "--print"):
            ...
            i += 1
            forced_ladder_tier = args[i] if i < len(args) else None
            if forced_ladder_tier not in LADDER_TIERS:
                print(f"[오류] --tier 값은 {'/'.join(LADDER_TIERS)} 중 하나: '{forced_ladder_tier}'",
                      file=sys.stderr)
                sys.exit(2)
        elif a == "--allow-dirty":
            allow_dirty = True
    ...
    if prompt is None:
        print("사용법: python cn_run.py -p \"<지시>\" [--model <alias|slug>] [--tier <ladder>] "
              "[--allow-dirty] [--readonly] | --set-model <alias|slug> | --reset-model | --list-models "
              "| --autoscale on|off|status | --explain -p \"...\"",
              file=sys.stderr)
        sys.exit(2)

    if explain_mode:
        cmd_explain(prompt, roster, override, forced_ladder_tier=forced_ladder_tier)
        return

    cmd_run(prompt, roster, override, base_config_arg, extra_format,
            forced_ladder_tier=forced_ladder_tier, allow_dirty=allow_dirty, readonly=readonly)
