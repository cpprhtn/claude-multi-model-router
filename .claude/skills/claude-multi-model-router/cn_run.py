...
def cmd_run(prompt, roster, override, base_config_arg, extra_format,
            forced_ladder_tier=None, allow_dirty=False):
    dirty = is_git_dirty()
...
def main():
    ...
    if prompt is None:
        print("사용법: python cn_run.py -p \"<지시>\" [--model <alias|slug>] [--tier <ladder>] "
              "[--allow-dirty] | --set-model <alias|slug> | --reset-model | --list-models "
    ...
    cmd_run(prompt, roster, override, base_config_arg, extra_format,
            forced_ladder_tier=forced_ladder_tier, allow_dirty=allow_dirty)
