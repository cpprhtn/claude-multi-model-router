...
Claude Code를 opusplan(플랜 모드 = Opus, 실행 = Sonnet)으로 쓰다 보면:

- 몇 줄짜리 수정에도 Sonnet 컨텍스트를 그대로 쓰는 게 아깝고,
- 이름 변경·포맷팅·보일러플레이트 같은 단순 반복 작업까지 Claude가 직접 편집할 필요는 없고,
- 요약·번역처럼 결과가 짧게 소비되는 작업은 무료 모델로도 충분한 경우가 많습니다.

이 스킬은 그 판단 기준을 규칙(SKILL.md의 티어 판정표)으로 못박아서, 세션마다 반복 설명하지 않아도 Claude가 알아서 적용하게 합니다.

## 티어 구조

기본값은 **Claude 모델끼리만 스케일링**합니다 — OpenRouter는 `route.py`(텍스트 생성)든 `cn_run.py`(파일 편집)든 아래에서 명시적으로 켜기 전까지 절대 자동으로 개입하지 않습니다. 둘은 같은 스위치를 공유합니다.

| 작업 | 실행처 |
|------|--------|
| 원인분석·설계·계획 (굵직한 요청) | 플랜 모드 (Opus) |
| 몇 줄 코드·짧은 답·단순 질의 | 메인 세션 직접 (Sonnet) |
| 여러 파일 수정·긴 출력 | Sonnet 서브에이전트 |
| 탐색·검색·대량 읽기 후 요점 회수 | Haiku 서브에이전트 |
| free 텍스트 생성 (요약/번역 등, 파일 편집 아님) — **OpenRouter 꺼짐(기본값)** | Haiku 서브에이전트 |
| free 텍스트 생성 — **OpenRouter 켜짐** | `route.py` — OpenRouter 모델 직접 호출 |
| 단순·기계적 파일 편집 (이름변경/치환/포맷팅) — **OpenRouter 꺼짐(기본값)** | Sonnet 서브에이전트가 직접 처리 |
| 단순·기계적 파일 편집 — **OpenRouter 켜짐** | `cn_run.py` — Continue CLI(`cn`)를 헤드리스로 구동해 위임 |

"OpenRouter 켜짐" 두 행 모두, sticky로 모델을 고정하지 않았다면 단순히 free 모델 하나를 고정 호출하는 게 아니라 **Claude의 티어 판정과 같은 원리로 요청 난이도를 다시 분류**해서 trivial(진짜 무료)~light~mid~reason 중 적합한 모델을 자동으로 고릅니다. `route.py`와 `cn_run.py`는 이 판단(sticky/autoscale 상태)을 공유합니다 — 유료 모델에 고정해두면 텍스트 생성 요청도 free로 새지 않고 그 모델을 씁니다.

OpenRouter를 켜려면 "오픈라우터 무료모델 써줘"(등급 없이 항상 free 고정) 또는 "오토스케일 켜줘"(난이도별 자동 선택, 어려우면 유료로도 감)라고 말하면 됩니다 — 둘 다 route.py·cn_run.py를 켜지만 동작은 다릅니다. 자세한 내용은 아래 "OpenRouter 위임 켜기" 참고. 세부 판정 로직과 비용 규칙은 [`.claude/skills/claude-multi-model-router/SKILL.md`](.claude/skills/claude-multi-model-router/SKILL.md)에 있습니다.
...
(저장소 루트)
  ...
  .claude/skills/
    claude-multi-model-router/
      SKILL.md              — 티어 판정 로직(진입점). Claude Code가 세션마다 읽는 스킬 정의.
      route.py               — 규칙 기반 난이도 분류(classify()) + OpenRouter 직접 호출.
                              파일 편집이 필요 없는 텍스트 생성/요약에 사용하며, 호출될 때는 Claude 토큰을
                              전혀 쓰지 않습니다. 이 호출 자체가 OpenRouter 위임(opt-in, 기본 꺼짐)에
                              속해 있어 — 꺼진 기본 상태에서는 이것도 쓰지 않고 Haiku가 대신 처리합니다.
                              sticky/autoscale 상태 관리의 원본이기도 해서(cn_run.py가 가져다 씀),
                              켜진 상태에서는 free만 호출하는 게 아니라 sticky/autoscale에 따라
                              유료 모델도 그대로 반영해서 호출합니다.
      cn_run.py               — `cn`(Continue CLI)을 헤드리스로 구동해 실제 파일 편집을
                              ...
                              필요할 때만 온디맨드로 유료 전환(1회성/sticky/오토스케일). 모델 로스터·
                              상태 관리 로직은 route.py 것을 그대로 가져다 씁니다(중복 구현 없음).
      models.json             — sonnet/opus ESCALATE 안내 라벨 전용(route.py가 호출하는 실제
                              모델 로스터가 아님 — 그건 cn_models.json).
      ...
      references/cn.md         — `cn` 위임 상세 가이드(헤드리스 패턴, 모델 전환, 폴백).
    model-picker/
...
수동으로 확인·조작하고 싶을 때(항상 프로젝트 루트에서 실행 — `cn_run.py`가 루트 `.env`에서 키를 찾기 때문):
...
`--explain`은 크레딧을 전혀 쓰지 않으므로, 오토스케일이나 sticky를 처음 만져볼 때 실제로 유료 모델을 부르기 전에 먼저 이걸로 확인해보는 걸 권장합니다.
